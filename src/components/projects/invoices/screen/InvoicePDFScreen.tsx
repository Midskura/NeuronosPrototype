...
export function InvoicePDFScreen({ project, invoice, onClose, currentUser, isEmbedded = false }: InvoicePDFScreenProps) {
  ...
  return (
    <div className={`flex flex-col h-full w-full bg-[#F3F4F6] overflow-hidden ${isEmbedded ? 'rounded-lg border border-gray-200' : ''}`}>
      ...
      {!isEmbedded && (
        <div className="h-16 bg-white border-b border-[#E5E9F0] px-6 flex items-center justify-between shrink-0 z-20 print:hidden">
            ...
            <div>
                <h2 className="text-xl font-bold text-[#12332B] tracking-tight">Invoice PDF Studio</h2>
                <p className="text-[11px] text-[#667085] font-mono leading-none mt-1">{invoice.invoice_number}</p>
            </div>
            </div>
        </div>
      )}
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        >
            ...
            <div className="absolute inset-0 overflow-auto flex items-center justify-center p-8 pb-32 print:p-0 print:overflow-visible print:absolute print:inset-0 print:block">
                ...
                <div className="print-portal-root flex flex-col items-center print:block print:w-full print:h-full">
                    ...
                    >
                        {/* The "Paper" Container */}
                        <div 
                            className="bg-white origin-top-left print:transform-none print:shadow-none print:static print:w-full print:h-full" 
                            style={{ 
                                width: '210mm', 
                                minHeight: '297mm',
            ...
            <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 bg-white/90 backdrop-blur-sm border border-gray-200 shadow-lg rounded-full px-4 py-2 flex items-center gap-4 z-30 transition-all hover:bg-white hover:shadow-xl print:hidden">
        ...
        <div className="w-[360px] bg-white border-l border-[#E5E9F0] flex flex-col overflow-hidden shrink-0 z-20 shadow-[-4px_0_15px_-3px_rgba(0,0,0,0.02)] print:hidden">
